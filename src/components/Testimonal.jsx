...
const TESTIMONIALS = [
  ...
  },
  {
    ...
    rating: 5,
    quote:  'The evening slot flexibility was what sold me. I booked sessions around my work schedule and cleared the RTO test on my very first attempt. Highly recommended.',
  ...
  },
  {
    ...
    rating: 5,
    quote:  'I had failed my test twice before joining. The Mastery Programme was a complete game-changer — structured, professional, and genuinely engaging. Passed with flying colours.',
        ...
        >
          {TESTIMONIALS.map(({ name, role, rating, quote, course }) => (
            <motion.article
              ...
              className="group flex flex-col gap-5 p-8 rounded-xl bg-bg border border-fg/15 hover:border-fg cursor-default transition-colors duration-300"
            >
              {/* Big quote mark */}
              ...
              <div className="flex gap-0.5">
                {Array.from({ length: rating }).map((_, i) => (
                  <span key={i} className="text-fg text-sm leading-none">★</span>
                ))}
              </div>

              {/* Quote text */}
              <p className="font-poppins text-sm leading-relaxed text-fg/65 flex-1">
                "{quote}"
              </p>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="font-poppins text-xs text-fg/45 mt-0.5">{role}</p>
                </div>
              </div>
            </motion.article>
          ))}
