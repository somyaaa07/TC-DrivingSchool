...
const LINKS = {
  ...
  Courses:  ['Foundation Drive', 'Confidence Course', 'Mastery Programme', 'Corporate Training'],
        ...
        >
          ...
          <div className="max-w-xs">
            ...
            <p className="font-poppins text-sm leading-relaxed text-fg/52 mb-8 -mt-6">
              Premium driving education since 2009. Building confident, safe drivers across NCR.
            </p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-10">
            {Object.entries(LINKS).map(([category, links]) => (
              <div key={category}>
                <p className="font-poppins text-[11px] tracking-[0.18em] uppercase text-fg/38 mb-5">
                  {category}
                </p>
                <ul className="flex flex-col gap-3">
                  {links.map((link) => (
                    <li key={link}>
                      <a
                        href="#"
                        className="group flex items-center gap-1 font-poppins text-sm text-fg/55 hover:text-fg transition-colors duration-200"
                      >
                        {link}
